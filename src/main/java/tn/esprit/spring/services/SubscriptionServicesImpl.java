...
public class SubscriptionServicesImpl implements ISubscriptionServices{
   ...
   // @Scheduled(cron = "* 0 9 1 * *") /* Cron expression to run a job every month at 9am */
    ...
    public void showMonthlyRecurringRevenue() {
        Float revenue = (subscriptionRepository.recurringRevenueByTypeSubEquals(TypeSubscription.MONTHLY) != null ? subscriptionRepository.recurringRevenueByTypeSubEquals(TypeSubscription.MONTHLY) : 0)
                + (subscriptionRepository.recurringRevenueByTypeSubEquals(TypeSubscription.SEMESTRIEL) != null ? subscriptionRepository.recurringRevenueByTypeSubEquals(TypeSubscription.SEMESTRIEL)/6 : 0)
                + (subscriptionRepository.recurringRevenueByTypeSubEquals(TypeSubscription.ANNUAL) != null ? subscriptionRepository.recurringRevenueByTypeSubEquals(TypeSubscription.ANNUAL)/12 : 0);
